...
with c1:
    gross_monthly = st.number_input("Monthly income (gross)", min_value=1_000,
                                    max_value=50_000, value=11_750, step=50)
    filing = st.radio("Filing status", ["single", "married"], horizontal=True)
with c2:
    household = st.number_input("Household size", 1, 10, 3)
    kids_u5 = st.number_input("Kids under 5 (childcare)", 0, 5, 1)
with c3:
    bedrooms = st.select_slider("Bedrooms", ["1br", "2br", "3br"], value="2br")
